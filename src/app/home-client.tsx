                        ...
                        {!bgProjectJobId && (
                            <div style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border-default)', borderRadius: '10px', padding: '12px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                ...
                                <button type="button" disabled={!bgProjectGoal.trim()} onClick={async () => {
                                    try {
                                        if (!bgProjectGithubRepo?.trim()) {
                                        const proceed = window.confirm('⚠️ No GitHub Repo set!\n\nWithout a GitHub Repo, files will only be written to your local bridge.\n\nEnter your GitHub repo for cloud execution (e.g. t4n-t3ch/t4n-ads).\n\nContinue without?');
                                        if (!proceed) return;
                                    }
                                    const { data: { session: s } } = await supabase.auth.getSession();
                ...
                {bgProjectTab === 'submit' && !bgProjectJobId && (
                    <button type="button"
                        ...
                        onClick={async () => {
                            if (!bgProjectGoal.trim()) return;
                            if (!bgProjectGithubRepo?.trim()) {
                                const proceed = window.confirm('⚠️ No GitHub Repo set!\n\nWithout a GitHub Repo, files will only be written to your local bridge (PC must be connected).\n\nTo use cloud execution (no PC needed), enter your GitHub repo first (e.g. t4n-t3ch/my-project).\n\nContinue without GitHub repo?');
                                if (!proceed) return;
                            }
                            if (!bridgeConnected) {
